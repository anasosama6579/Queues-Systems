...
class CustomerNumberChart extends StatelessWidget {
  const CustomerNumberChart({super.key, required this.info});
...
List<Map<String, dynamic>> spots({required double lambda, required double mu, required double time}) {
  ...
  for (var xSpot in xSpots) {
    xSpot % lambda == 0 ? isArrival = true : isArrival = false;
    ((xSpot - lambda) % mu == 0 && (xSpot - lambda) / mu != 0)
        ? isServed = true
        : isServed = false;
    if (isArrival) ySpot++;
    if (isServed) ySpot--;
  ...
  spots.add(
      {'isArrival': false, 'isServed': false, 'xSpot': time, 'ySpot': ySpot});
  return spots;
}
